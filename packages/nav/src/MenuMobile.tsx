...
export const MenuMobile = ({
  ...
  const ButtonCloseNav = (classNames?: string) => {
    return (
      ...
      >
        <svg className={cn(baseStyles.menuIcon, 'h-6 w-6')} fill="none" viewBox="3 6 18 12">
          {isMobileMenuOpen ? (
            <path
              className={cn(baseStyles.nav, variantBoxMobileStyles[variantBoxMobile][variant].x)}
              strokeLinecap="round"
      ...
      >
        <div className="mt-2">
          <div className="relative">
            {LogoNavMenuMobile && <div className={baseStyles.logo}>{<LogoNavMenuMobile />}</div>}
          </div>
