...
export const ProductGallery: React.FC<ProductGalleryProps> = ({
  ...
  const currentIndex = images.gallery.findIndex(img => img.id === selectedImage.id)
  ...
  const thumbsClassName = `
    ${thumbPosition === 'left' ? 'flex flex-col gap-4 w-24' : 'grid grid-cols-4 gap-4'}
  `
        ...
        <div className={mainImageClassName}>
          <div className="relative">
            <div
              ref={setRefs}
              {...swipeHandlers}
              className="aspect-square cursor-zoom-in overflow-hidden rounded-lg bg-gray-100"
            ...
            {/* Zoom Window */}
            {showZoom && (
              <div className="absolute left-[100%] top-0 ml-4 hidden h-[400px] w-[400px] overflow-hidden rounded-lg border-2 border-gray-200 bg-white lg:block">
                <div
      ...
      {showModal && (
        ...
        >
          <div className="absolute inset-0 flex items-center justify-center">
            <button
              className="absolute right-4 top-4 bg-black bg-opacity-40 p-1"
              onClick={() => setShowModal(false)}
              title="Cerrar (ESC)"
            >
              <svg className={cn('h-6 w-6')} fill="none" viewBox="0 0 24 24">
                <path
                  className={cn('h-full w-full stroke-white')}
                  strokeLinecap="round"
            ...
            >
              <div className={cn('h-6 w-6')}>
                <svg
                  ...
                  viewBox="0 0 16 30"
                  fill="none"
                  className={cn('h-full w-full fill-white')}
            ...
            <img
              src={selectedImage.size.lg.url}
              alt={selectedImage.alt}
              className="object-contain md:max-h-screen md:max-w-screen-lg"
            />
            ...
            {/* Counter */}
            <div className="absolute left-4 top-4 rounded bg-black bg-opacity-40 px-3 py-1 text-sm text-gray-400">
